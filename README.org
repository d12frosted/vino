...
In order to build database you may manually call =vino-db-sync=, but in case you
properly [[#installing][installed]] =vino= (e.g. called =vino-setup=), then =vino-db-sync= will
be called each time =org-roam-db-build-cache= is called (e.g. respecting
